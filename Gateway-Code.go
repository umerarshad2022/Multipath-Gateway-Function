...
var externalIP = "100.87.50.1"           // External IP address of the gateway 
...
func main() {
    ...
    go func() {
        defer wg.Done()
        startListener("192.168.10.2:8080", handleHostConnection)
    }()
    ...
    go func() {
        defer wg.Done()
        startListener("100.87.50.1:8080", handleServerConnection)
    }()
...
func handleHostConnection(conn net.Conn) {
    ...
    for {
        ...
        translatedData := performNAT(ipPacket, tcpPacket, "192.168.10.2", externalIP)
...
func handleServerConnection(conn net.Conn) {
    ...
    for {
        ...
        translatedData := performNAT(ipPacket, tcpPacket, externalIP, "192.168.10.2")
...
func forwardToServer(packet []byte) {
    conn, err := dialMPTCP("100.87.50.2:8080", "ens8") // Primary interface for MPTCP
    if err != nil {
...
func forwardToHost(packet []byte) {
    conn, err := net.Dial("tcp", "192.168.10.1:8080")
    if err != nil {
